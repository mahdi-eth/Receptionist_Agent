...
class CreateReservationTool(BaseTool):
    ...
    reservation_service: ReservationService = Field(exclude=True)  # Exclude from serialization
...
class HotelAgent:
    ...
    def _init_tools(self):
        ...
        self.tools = {
            ...
            "create_reservation": CreateReservationTool(reservation_service=self.reservation_service)
        }
    ...
    async def create_session(self, session_id: str, guest_info: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Created session {session_id} with guest info: {guest_data is not None}")
            return True
    ...
    async def _process_with_graph(self, state: AgentState, db: AsyncSession) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    def _analyze_message_simple(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(word in last_message.lower() for word in ["book", "reserve", "reservation"]):
                state["current_task"] = "booking"
            elif any(word in last_message.lower() for word in ["room", "available", "check"]):
                state["current_task"] = "room_inquiry"
            elif any(word in last_message.lower() for word in ["name", "email", "info"]):
    ...
    async def _generate_response_simple(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            state["response"] = "I apologize, but I'm having trouble processing your request right now. Could you please rephrase your question?"
            return state
